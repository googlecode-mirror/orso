...
CREATE TABLE activity_concept (activity_id BIGINT, concept_id BIGINT, PRIMARY KEY(activity_id, concept_id)) ENGINE = INNODB;
...
CREATE TABLE user (id BIGINT AUTO_INCREMENT, username VARCHAR(255) NOT NULL UNIQUE, email VARCHAR(255) UNIQUE, group_id BIGINT, INDEX group_id_idx (group_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE user_group (id BIGINT AUTO_INCREMENT, name VARCHAR(255) UNIQUE, PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE user ADD FOREIGN KEY (group_id) REFERENCES user_group(id) ON DELETE CASCADE;
