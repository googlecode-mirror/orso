    ...
    <table id="main_list" cellspacing="0">
      <thead>
        <tr>
          ...
          <?php include_partial('domain/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <tr>
          <th colspan="5">
            <?php if ($pager->haveToPaginate()): ?>
              <?php include_partial('domain/pagination', array('pager' => $pager)) ?>
            <?php endif; ?>
      ...
      <tbody>
        <?php foreach ($pager->getResults() as $i => $domain_model): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
        <tr id="node-<?php echo $domain_model['id'] ?>" class="sf_admin_row <?php echo $odd ?><?php
          // insert hierarchical info
          $node = $domain_model->getNode();
          if ($node->isValidNode() && $node->hasParent())
          {
            echo " child-of-node-".$node->getParent()->getId();
          }
          ?>">
          <?php include_partial('domain/list_td_batch_actions', array('domain_model' => $domain_model, 'helper' => $helper)) ?>
          <?php include_partial('domain/list_td_tabular', array('domain_model' => $domain_model)) ?>
          <?php include_partial('domain/list_td_actions', array('domain_model' => $domain_model, 'helper' => $helper)) ?>
          </tr>
<?php endforeach; ?>
